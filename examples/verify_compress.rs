// examples/verify_compress.rs
// SPDX-License-Identifier: CC0-1.0
// This file was created entirely or mostly by an AI tool: claude-opus-4-6
//
//! End-to-end compression verification: compresses files with
//! `ParBz2Encoder`, decompresses with the reference `bzip2::read::BzDecoder`,
//! and verifies the output matches the original input byte-for-byte.
//!
//! Usage:
//!     cargo run --example verify_compress -- file1 file2 ...
//!
...
/// Returns (original_bytes, compressed_bytes, compress_ms).
fn verify_file(path: &Path, pb: &ProgressBar) -> Result<(u64, u64, u128), String>
{
	...
	// ── Pass 2: decompress with reference decoder + compare ────
	pb.set_position(0);
	pb.set_length(original_size);
	pb.set_message("verify");

	let mut ref_decoder = bzip2::read::BzDecoder::new(Cursor::new(&compressed));
	...
	loop {
		...
		let dec_n = read_exact_or_eof(&mut ref_decoder, &mut dec_buf).map_err(|e| format!("ref decode: {e}"))?;
	...
	pb.finish_and_clear();
	Ok((original_size, compressed_size, compress_ms))
}
...
fn main()
{
	...
	println!("Verifying compression of {total} file(s), up to {MAX_CONCURRENT} concurrently\n");
